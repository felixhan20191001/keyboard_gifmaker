...
**Outfit lock (required — Felix 2026-09-15 hard rule):** clothing must stay **highly consistent** with the **user-uploaded character/outfit reference for this run** (the image Felix attaches each time as the character — same garment type, cut, colors, embroidery/jewelry layout, materials). **Not** the skill pose reference (`refs/motion-a-recline-pose-ref.png` / pose-ref / pose-edges): those are pose/composition only — never copy their outfit. Do not invent a different dress, swap costume class, or restyle major outfit regions. Pass the **user-uploaded character-ref** path **twice** on image_edit/image_gen for identity+outfit lock. Self-check fails and re-edits when outfit drifts from that uploaded reference.

**Garment physics (required — Felix 2026-09-15 hard rule):** **Only when the costume includes a skirt / gown / dress / long robe** (not shorts, pants, bodysuit, bikini, or other non-skirt outfits — those **ignore** this coverage rule): the skirt fabric must **cover the hips and the thighs** (drape over the thigh surface in frame — not hiked to the crotch), then pool by gravity in natural folds (Motion A: whatever ground/backdrop is in the still). **Never show underwear, panties, briefs, or bare groin under a hiked skirt.** Do not wedge skirt fabric between the legs so it reads as shorts, a leotard, or a bikini bottom. A high slit may show a **lower** thigh/leg only if the source costume already has one **and** underwear stays fully covered. Reject and re-edit any **skirted** still where underwear is visible or the skirt fails to cover the thighs.

Inspect the new still: if feet, head, or hands are missing, **outfit drifted from character-ref**, the gown is bunched as underwear, **or (if skirted) underwear is visible / the skirt does not cover the thighs**, a staff is still in a free near hand, she is hovering on two straight arms, or identity drifted, edit once more. Motion A: also reject a pose that is not the **chin-hand recline** (missing chin-rest support hand, free hand not on thigh/hip, pure side silhouette with hips not twisted toward camera, or a Motion B tucked-elbow supermodel recline). Motion B: also reject a pose that is not the supermodel recline. Motion C: also reject a pose that is not prone with calves lifted, a camera that is not a side view, a profile head, or a face that is not turned toward the camera. Motion A: do **not** reject on background. Motion B: do not reject the original scene. Motion C: do not reject a simple dark floor. Only then set `$FULLBODY_STILL` to this generated file.
...
4. Loop back to the still (三点「循环」／first=last)
...
**Loop (required on web Imagine):** `$FULLBODY_STILL` as start; **`loop_via_image_menu: true`** (三点「循环」only — do not also Add last frame). No timed 5.4 s pose reset.

**Default Imagine `final_prompt` template (English; paste into the plan):**

```text
Use the uploaded still as composition + identity lock. Enable thumbnail three-dot 「循环／Loop」 so this exact image is BOTH first and last frame. Do NOT use separate Add last frame.
...
**Motion A:** Pixel-crop `*-face.png`. **Do not** make `*-curl.png` unless Felix asks for legacy come-hither. On **Grok web Imagine**, run one 16:9 ~6 s Video with Uploads of `$FULLBODY_STILL` (and face if slots allow). **`loop_via_image_menu: true`** — 三点只开「循环」, do not also Add last frame. (In Grok Build TUI, approximate with `reference_to_video` 16:9 6 s, not a face-only `image_to_video`.)
...
**Prompt = deltas only** (see Motion A **Default Imagine `final_prompt` template** above). Do **not** re-describe the recline / chin-rest pose. Copy the MP4 to `$RUN_DIR/*-source.mp4`.
...
Reject a zero-byte, incomplete, static, or whole-frame-only scale/translation source. Stations must prove **locked Motion A framing + micro head-turn/smile timing + identity + native loop**.
...
- **Last frame:** matches `$FULLBODY_STILL` / first frame (三点 Loop). Do **not** fail only because a mid timestamp is not yet on the home pose.
...
**Motion A:** first and last frames must be the **same chin-hand recline crop** as `$FULLBODY_STILL` (pose-ref mid-recline OK; feet not required; background unrestricted); a mid-clip frame must keep that same locked crop with matching garment; inspect frames **between** the quartiles for the **head-turn and/or soft smile** beat — that beat will not sit on the first/last samples. The face/head change must remain readable at 320×172.
