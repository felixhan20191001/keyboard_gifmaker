...
1. **Recumbent come-hither** — one locked-camera 16:9 full-body shot; she lies across the wide screen in the **Motion A chin-hand recline** (torso twisted toward camera, supporting hand under chin, free hand beckons) and hooks an index finger toward the camera. Pose reference: `skills/generating-rt85-gifs/refs/motion-a-recline-pose-ref.png`.
...
- **Usable already (A):** head, hair, both hands, torso, legs, and feet (plus cape, tail, or other attached props) are all visible with small margins; she is in the **Motion A chin-hand recline** (torso and hips twisted toward the camera, supporting elbow on the ground with that hand under the chin, free hand on the upper thigh/hip ready for come-hither); landscape 16:9; **flat solid #888888 fill** with no original scene and **no bench, slab, or board** (the pose-ref bed/moon scene is **not** kept unless the user asked).
...
- **Not usable:** standing, seated, half-body, chest-up, missing feet, portrait crop, or the figure is tiny in a wide empty frame. Motion A is also not usable when the original forest / throne / architecture / crowd is still behind the figure, the figure is lying on a bench, stone slab, or board, or the background is a cyclorama, studio gradient, or textured gray rather than a flat solid #888888 fill. **Do not animate this crop.** First generate a new **16:9 full-body illustration** of the **same character** with `image_edit` (preferred in Grok Build). Pass the reference **twice** so 16:9 is honored. Preserve exact face, hair, outfit, **body type**, and style. Complete the unseen lower body consistently with the visible costume. Motion A: use the chin-hand recline below (not Motion B's supermodel recline). Motion B: keep the source background; do not replace it with #888888. Motion C: use the prone side-view pose below, not the A/B recline.

**Pose (required, Motion A) — chin-hand recline (Felix 2026-09-14):** match the body language of `refs/motion-a-recline-pose-ref.png`, adapted to a **16:9 full-body** (hair/crown through feet, small margins, character fills the frame). She lies across the screen with **head toward the left** and **feet toward the right**, but this is **not** a pure side silhouette: **torso and hips twist toward the camera** so the chest and near thigh read clearly. **Supporting arm:** the near/down elbow rests on the ground; that hand is lightly **under the chin** (chin rest), framing the face. **Free arm:** hand rests on the **upper thigh / hip** in the home pose — this is the come-hither hand. **Legs:** both softly bent; the top knee sits higher so the near hip/thigh is prominent in the foreground. Eyes **open**, looking at the camera. She lies **directly on the flat solid #888888 ground** (do not copy the pose-ref bed, moon, pavilion, or props unless the user asked). Never overhead. Never two locked arms pushing the torso up. Do not put a staff, weapon, or other prop in the free hand; omit it unless the user asked to keep it. Do **not** use Motion B's tucked-elbow supermodel recline or Motion C's prone calf-lift as Motion A's still.

**Pose-ref on every Motion A still (required):** whenever generating or fixing a Motion A `$FULLBODY_STILL` / face / curl still, **always** pass `refs/motion-a-recline-pose-ref.png` as an extra reference image together with the character reference — for both primary `GenerateImage` and Grok Build `image_edit` fallback. Use the pose-ref for **body language / hand placement / hip twist only**; lock face, hair, outfit, and body type to the **character** reference. Never invent Motion A pose from text alone when this file is available.
...
`$FULLBODY_STILL` is the **Motion A chin-hand recline** home pose (see pose section + `refs/motion-a-recline-pose-ref.png`): chin-rest support hand stays put; the **thigh/hip hand** is the beckon hand. Pixel-crop `*-face.png` into 16:9 (hair inside the top edge, chin / neckline in the lower third; the support hand under the chin may stay in frame). Do **not** default to `image_edit` for this crop: a redraw often changes the face. Composite onto matching #888888 if the source has alpha. Only use `image_edit` if a crop cannot reach 16:9 without cutting the subject; then pass the still twice so 16:9 is honored, and reject the result if identity drifted.

Also make `*-curl.png` from `$FULLBODY_STILL`: **same chin-hand recline framing and identity**, only the **free (thigh) hand** changes — it lifts to **collarbone-to-chest height** with **index finger hooked inward** in a come-hither, other fingers softly curled, palm toward the camera. The chin-rest hand does **not** move. Video models skip the curl unless they see this still. Reject a curl still that is a wave, a point-at-the-sky, a salute, both-eyes-closed, or that drops the chin-rest support.

The chin-hand-recline full-body still is the **loop start and end**. Do not start the clip on a feet close-up.

**Loop (required on web Imagine):** use **`$FULLBODY_STILL` as both the start image and Add last frame** (exact same file). Do **not** require a timed return to the starting pose at 5.4 s or a protected hold tail — seamless looping comes from that first=last still.
...
**Come-hither:** the **thigh/hip hand** lifts to **collarbone-to-chest height**, palm toward the camera. Over the clip the **index finger curls inward** (match `*-curl.png`), then relaxes back toward the thigh, **two or three times**. The **chin-rest support hand stays under the chin** for the whole loop. Eyes stay **open** and look at the camera. Do not turn the gesture into a wave, a salute, or a point at the sky. Do not flatten into a pure side-view supermodel recline.
...
**Face:** expression **changes** mid-clip and is readable at 320×172 — a shifting smile, a playful beat — not a frozen mask. Eyes stay open (a brief mid-clip blink is allowed). Do **not** require the face to return to the first-frame look by 5.4 s.
...
**Loop (required):** first frame and last frame are this same `$FULLBODY_STILL`. On **Grok web Imagine**, upload `$FULLBODY_STILL` as the start image **and** set **Add last frame** to the **exact same file**. Do **not** require a 0.4 s start hold or a timed return by 5.4 s — the native first=last still closes the loop. If that ~6 s MP4 first/last already match, even-sample into the GIF (**≤51 frames**). If the **end hold is long**, drop extra still frames and keep a short home hold — duration follows what remains. Do not generate an open-ended clip. Do not fake the seam by copying the first frame onto the last or by fading.
...
**Motion C:** do not animate a standing, seated, overhead, or A/B side-recline crop. After `$FULLBODY_STILL` is the approved prone side-view, on **Grok web Imagine** run one 16:9 ~6 s Video from that still with **start image and Add last frame = the exact same `$FULLBODY_STILL`**. (In Grok Build TUI, `image_to_video` / `reference_to_video` with the same still twice.) The prompt must require: lock the side camera on the exact full-body framing of `$FULLBODY_STILL` from first frame to last; she stays prone on the ground; calves kick playfully back and forth; the upper body does **simple continuous motion** — a forearm weight shift, a slight head turn, a tiny shoulder roll — and must **not look frozen**; **facial expression changes** mid-clip; seamless loop closes on `$FULLBODY_STILL` via Add last frame (do **not** require a 0.4 s start hold or a timed return by 5.4 s); no sit-up, crawl, stand, dance, zoom, or pan; eyes stay open. Copy the MP4 to `$RUN_DIR/*-source.mp4`. Do not run Motion A's loop-search trim on this clip.
...
Reject a zero-byte, incomplete, static, or whole-frame-only scale/translation source. Stations must prove **locked full-body framing + curl timing + identity + native loop**.

- **~0 s:** 16:9 **chin-hand recline full body** lying **on the flat solid #888888 ground**, eyes open, support hand under chin, free hand on thigh/hip (beckon home). Same crop as `$FULLBODY_STILL`. No bench, slab, board, or pose-ref bedroom scene.
...
- **~0 s and last frame:** same 16:9 **prone side-view full body** as `$FULLBODY_STILL`, calves lifted, same crop. Camera has not moved. Last frame matches the first-frame calf angle, arms, head, and hair (Imagine Add last frame).
...
- Do **not** require a timed return to the start pose by 5.4 s; only first and last must match the still.
...
**Motion A:** the source should already loop via Imagine Add last frame (first≈last). Prefer the even-sample path, then drop extra still tail if needed. If it does not loop, **regenerate** with the same `$FULLBODY_STILL` as start and last frame rather than relying on a pixel-best seam. A fallback trim/assemble via `search_rt85_loop.py` is allowed only as a last resort (prefer the **longest** window at **320×172** whose blurred endpoint MAE is ≤ 16 and whose seam is either ≤ **1.3×** median consecutive-frame MAE or ≤ 12 absolute RGB MAE), **≤51 frames** @ 10 fps. Do not add fade crossfades or static fake animation. Do **not** require a 5.4 s pose reset.
...
**Motion C:** do not run `search_rt85_loop.py`. The ~6 s source must already loop (Gate 1 last frame is home via Add last frame). Even-sample ≤51 frames from t=0 through the last source frame, scale/crop to 320×172, then **drop extra still frames at the end** if that hold is long — keep 1–2 home frames. Duration is whatever remains. Palette-GIF (`loop=0`). If first/last of the source do not match, regenerate with the same still as start and last frame; do not even-sample a jumping clip and do not copy the first frame onto the tail. Do **not** require a timed 5.4 s reset.
...
**Motion A:** first and last frames must be the **same full-body chin-hand recline crop** lying **on the flat solid #888888 ground** (chin-rest hand + thigh home hand); a mid-clip frame must still show the whole lying figure with matching garment and #888888 fill, **no bench / slab / board**; inspect frames **between** the quartiles for the index-finger curl from the free hand — that beat will not sit on the first/last samples. The curl must remain readable at 320×172.
