...
**Outfit lock (required — Felix 2026-09-15 hard rule):** clothing must stay **highly consistent** with the character reference (same garment type, cut, colors, embroidery/jewelry layout, materials). Do not invent a different dress, swap costume class, or restyle major outfit regions. Pass the character-ref path **twice** on image_edit/image_gen for identity+outfit lock. Self-check fails and re-edits when outfit drifts from the reference.
